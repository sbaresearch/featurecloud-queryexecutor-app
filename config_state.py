...
class State(AppState):
    ...
    def __init__(
        self, app_name, input_dir: str = "/mnt/input", output_dir: str = "/mnt/output"
    ):
        self.config = {}
    ...
    def read_config(self):
        ...
        if "logic" in self.config:
            ...
            self.log(
                f"There are no 'logic' options in 'config.yml' file!\n"
    ...
    def finalize_config(self):
        ...
        if self.mode == "directory":
            splits = [
                f.path for f in os.scandir(f"{self.input_dir}/{self.dir}") if f.is_dir()
            ]
        else:
            splits = [
                self.input_dir,
        ...
        self.store(
            ...
            {
                k: [
                    f"{split.replace('/input', '/output')}/{v}"
                    for split in self.load("splits")
                ]
                for k, v in self.config["results"].items()
            },
        )
        for split in self.load("splits"):
